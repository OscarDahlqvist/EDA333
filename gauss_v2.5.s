...
.eqv AKKSTEP 	$v0 # the number of bytes you must offset to reach the (1 right, 1 down) diagonal from a given space
.eqv MATRIXEND 	$v1 # the first memory adress after the matrix (this and AIK_MAX will store adresses outside the matrix, but these are never read/written)
# </aliases>
