...
eliminate:
		...
		# $s3 = float* adressof A[k][k]
		# $f3 = float A[k][k]
		
		# $s4 = float* adressof A[i][k]
		# $f4 = float A[i][k]
		
		# $s5 = float* adressof A[i][j]
		# $f5 = float A[i][j]

		l.s $f1, float1.0
				
			move $s0, $0 # k = 0	
doWhile_k:	# for (k = 0; k < N; k++) 
			# TODO use a2 (row num) and a3 (colum num) directly to not move arounds the vars
			
			move $a2, $s0
			move $a3, $s0
			jal getElem
			move $s3, $v0 	# s2 = &A[k][k]
			mov.s $f3, $f0 	# f1 = A[k][k]
			
				add $s1,$s0,1 # j = k+1
				b for_j_CMP
for_j:			# for (j = k+1; j < N; j++) 
				move $a2, $s0
				move $a3, $s1
				jal getElem
				# f0 = A[k][j]
				...
				div.s $f0, $f0, $f3 # $f0 <- A[k][j] / A[k][k]
				s.s $f0,0($v0)		# A[k][j] <- f0

for_j_ITER:		add $s1,$s1,1		# j++
for_j_CMP:		slt $t0,$s1,$a1		# t0=1 if $s1 < $a1 (j<N)
				beq $t0,1,for_j		# if j < N
				
			s.s $f1, 0($s3) 	# A[k][k] <- 1.0f
			
				add $s2,$s0,1 # i = k+1
				b for_i_CMP
for_i:			# for (i = k+1; i < N; i++) 
				move $a2, $s2 	# i -> a2
				move $a3, $s0 	# k -> a3
				jal getElem
				move $s4, $v0 	# s4 = adressof A[i][k]
				mov.s $f4, $f0 	# f4 = A[i][k]
				
					add $s1,$s0,1 # j = k+1
					b for_j2_CMP
for_j2:				# for (j = k + 1; j < N; j++)
					move $a2, $s2 	# i -> a2 (already set the first time but not subsequent)
					move $a3, $s1 	# j -> a3 
					jal getElem
					move $s5, $v0 	# s5 = adressof A[i][j]
					mov.s $f5, $f0 	# f5 = A[i][j]
					
					move $a2, $s0 	# k -> a2
					#move $a3, $s1	# j -> a3 (Redundant since A[i][j] and A[k][j] has the same column index
					jal getElem
					# f0 = A[k][j]
					# v0 = adressof A[k][j]
					
					mul.s $f0, $f0, $f4 # f0 <- A[k][j] * A[i][k]
					sub.s $f0, $f5, $f0 # f0 <- A[i][j] - f0
					s.s $f0, 0($s5)		# f0 -> A[i][j]
					
for_j2_ITER:		add $s1,$s1,1		# j++
for_j2_CMP:			slt $t0,$s1,$a1		# t0=1 if $s1 < $a1 (j<N)
					beq $t0,1,for_j2	# if i < N
					
				sw $0, 0($s4) # direcly set A[i][k] to 0.0f (works since 0x0 = 0.0f)
				
for_i_ITER:		add $s2,$s2,1		# i++
for_i_CMP:		slt $t0,$s2,$a1		# t0=1 if $s2 < $a1 (i<N)
				beq $t0,1,for_i		# if i < N
			
			#

			add $s0,$s0,1		# k++
doWhile_k_CMP:	
			slt $t0,$s0,$a1		# t0=1 if $s0 < $a1 (k<N)
			beq $t0,1,doWhile_k		# if k < N
